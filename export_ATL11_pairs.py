...
def export_ATL11_pairs(input_dir, output_dir, regex, CYCLES=None, EPSG=None,
	...
	#-- valid tracks
	valid_tracks = np.zeros((1387),dtype=np.bool)
	valid_tracks_cycles = np.ones((1387,3),dtype=np.bool)
	#-- for each input file
	for input_file in sorted(input_files):
		...
		for i,p in enumerate([1, 2, 3]):
			...
			if SUBSETTING:
				...
				#-- set track to true
				valid_tracks[np.int(TRK)-1] |= int_test
				#-- if there are points encapsulated
				if int_test and cycle_test:
					...
					for j,c in enumerate(CYCLES):
						...
						#-- set track to true
						valid_tracks_cycles[np.int(TRK)-1,i] &= np.any(D11m[p])
						if FILTER:
				...
				#-- set track to true
				valid_tracks[np.int(TRK)-1] = True
	...
	ind1, = np.nonzero(valid_tracks)
	ind2, = np.nonzero(np.any(valid_tracks_cycles,axis=1))
	print(sorted(set(ind1+1) - set(ind2+1)))
